...
function init()
{
	...
	success:function(data){
		...
		for(var i=0;i<num;i++)
		{
			var a=data[i];
			songs[i]=new songObj();
			songs[i].init(a['name'],a['image_id'],a['singer'],a['lyr'],a['id'],a['islike']);
		}
		songs[index].update();
		showLyric();
		drawVol(0.5);
...
function creatLocal(arr)
{
	...
	var flag=0;//用于判断歌曲是否存在于歌单，不存在则需插入数据库
	var a=arr;
	a=a.split(",");
	console.log(a);
	for(var i=0;i<num;i++)
	...
	if(flag==0)
	{
       ...
       $.ajax({
		type: 'POST',
		url: 'http://139.199.64.191:8080/player/creat.jsp?arr='+arr,
		success: function(msg){
			console.log("hi");
				arr=arr.split(",");
		      	songs[num]=new songObj();
		      	songs[num].init(arr[1],arr[4],arr[3],arr[5],arr[0],0);
		      	index=num;
		      	num++;
		      	 islyric=false;
		     	var html=document.getElementById("lyric_button");
...
function showLyric()
{
	...
	if(islyric==true)
	{
		...
		var id=songs[index].id,
   		txt = 'https://api.hibai.cn/music/Qmusic/Music?id='+songs[index].lyr+'&type=lrc';
   		 $.ajax({
   		 	type:"post",
   		 	url:txt,
   		 	datatype:"xml",
